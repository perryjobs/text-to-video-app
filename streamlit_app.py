...
# Use smaller size for preview but maintain aspect ratio 9:16 for vertical
if fmt == "Vertical":
    W, H = 540, 960  # smaller than 1080x1920 for faster preview
else:
    W, H = 720, 720
...
if st.button("Generate Video"):
    ...
    # Create quote clips with text animation over backgrounds
    for i, q in enumerate(quotes):
        bg = bg_clips[i % len(bg_clips)].set_position("center").set_duration(quote_dur)
        txt_clip = animated_text_clip((W, H), q, font, text_color, text_anim, quote_dur).set_duration(quote_dur)
        comp = CompositeVideoClip([bg, txt_clip], size=(W, H))
        clips.append(comp)

    if not clips:
        st.error("No clips created, please check inputs.")
        st.stop()
    ...
    st.download_button("Download Video", open(out, "rb"), "quote_video.mp4")
